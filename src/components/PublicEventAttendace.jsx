...
const AttendanceList = ({ eventId }) => {
    ...
    const fetchAttendanceList = async () => {
      try {
        ...
        if (response.ok) {
          ...
          console.log('data:', data.attendances);
        } else {
  ...
  const handleSendEmails = async () => {
    try {
      const response = await fetch('/api/event/sendAttendanceEmails', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (response.ok) {
        alert('Correos enviados exitosamente a los asistentes');
      } else {
  ...
  return (
    <div className="container mx-auto p-4 mt-12">
      ...
      <div className="overflow-x-auto">
        <div className="mb-8 flex flex-1 justify-end">
          <button
            onClick={handleSendEmails}
            className="bg-teal-500 text-white px-4 py-2 rounded-lg font-semibold hover:bg-teal-600 transition-all"
          >
        ...
        <table className="min-w-full bg-white border border-gray-200 rounded-lg shadow-lg">
          <thead>
            <tr className="bg-gray-200">
              <th className="py-2 px-4 border-b text-left text-sm font-semibold uppercase tracking-wider">
                Estudiante
              </th>
              <th className="py-2 px-4 border-b text-left text-sm font-semibold uppercase tracking-wider">
                Estado
        ...
        <div className="fixed inset-0 bg-gray-800 bg-opacity-75 flex items-center justify-center">
          <div className="bg-white p-4 rounded shadow-md w-1/3">
            <h3 className="text-xl font-semibold mb-4">
              ¿Confirmar cambio de asistencia para Hector Zatarain?
            </h3>
